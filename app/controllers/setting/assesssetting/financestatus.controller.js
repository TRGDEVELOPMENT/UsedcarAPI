...
exports.getFinanceStatusList = async (req, res) => {
  ...
  if (key != "") {
    sqlCommand =
      "SELECT ENFINANCESTATUS.ID ID, ENFINANCESTATUS.NAME NAME, " +
      "ENFINANCESTATUS.DESCRIPTION DESCRIPTION, CASE WHEN ENFINANCESTATUS.ISACTIVE = TRUE THEN 1 ELSE 0 END AS ISACTIVE, " +
      "ENFINANCESTATUS.CREATORID CREATORID, ENFINANCESTATUS.CREATORNAME CREATORNAME, " +
      "ENFINANCESTATUS.CREATEDDATE CREATEDDATE, ENFINANCESTATUS.MODIFIEDID MODIFIEDID, " +
      "ENFINANCESTATUS.MODIFIEDNAME MODIFIEDNAME, ENFINANCESTATUS.MODIFIEDDATE MODIFIEDDATE, " +
      "ENFINANCESTATUS.DELETEDID DELETEDID, ENFINANCESTATUS.DELETEDNAME DELETEDNAME, " +
      "ENFINANCESTATUS.DELETEDDATE DELETEDDATE " +
      "FROM OSUSR_X6Q_FINANCESTATUS ENFINANCESTATUS " +
      "WHERE (ENFINANCESTATUS.ISACTIVE = 1) AND (ENFINANCESTATUS.NAME LIKE '%key%') " +
      "ORDER BY ENFINANCESTATUS.ID DESC LIMIT 5 OFFSET @offset";
    ...
    sqlCommand =
      ...
      "ENFINANCESTATUS.DELETEDDATE DELETEDDATE " +
      "FROM OSUSR_X6Q_FINANCESTATUS ENFINANCESTATUS " +
      "WHERE (ENFINANCESTATUS.ISACTIVE = 1) " +
      "ORDER BY ENFINANCESTATUS.ID DESC LIMIT 5 OFFSET @offset";
  ...
  if (key != "") {
    countsql =
      "SELECT COUNT(ENFINANCESTATUS.ID) AS Count " +
      "FROM OSUSR_X6Q_FINANCESTATUS ENFINANCESTATUS " +
      "WHERE (ENFINANCESTATUS.ISACTIVE = 1) AND (ENFINANCESTATUS.NAME LIKE '%key%') " +
      "ORDER BY ENFINANCESTATUS.ID DESC";
...
exports.updateFinanceStatusById = async (req, res) => {
  const jsonData = JSON.parse(JSON.stringify(req.body));
  const timestamp = new Date().toISOString().slice(0, 19).replace("T", " ");
  const sqlCommand = "UPDATE OSUSR_X6Q_FINANCESTATUS SET ? WHERE ? ";
...
exports.deleteFinanceStatusById = async (req, res) => {
  const jsonData = JSON.parse(JSON.stringify(req.body));
  const timestamp = new Date().toISOString().slice(0, 19).replace("T", " ");
  const sqlCommand = "UPDATE OSUSR_X6Q_FINANCESTATUS SET ? WHERE ? ";
...
exports.insertFinanceStatus = async (req, res) => {
  const jsonData = JSON.parse(JSON.stringify(req.body));
  const timestamp = new Date().toISOString().slice(0, 19).replace("T", " ");
  const sqlCommand = "INSERT INTO OSUSR_X6Q_FINANCESTATUS SET ? ";
