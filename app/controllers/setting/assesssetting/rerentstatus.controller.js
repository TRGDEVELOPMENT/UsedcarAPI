...
exports.getReRentStatusList = async (req, res) => {
  ...
  if (key != "") {
    sqlCommand =
      "SELECT ENRERENTSTATUS.ID ID, ENRERENTSTATUS.STATUS STATUS, " +
      "ENRERENTSTATUS.DESCRIPTION DESCRIPTION, CASE WHEN ENRERENTSTATUS.ISACTIVE = TRUE THEN 1 ELSE 0 END AS ISACTIVE, " +
      "ENRERENTSTATUS.CREATORID CREATORID, ENRERENTSTATUS.CREATORNAME CREATORNAME, " +
      "ENRERENTSTATUS.CREATEDDATE CREATEDDATE, ENRERENTSTATUS.MODIFIEDID MODIFIEDID, " +
      "ENRERENTSTATUS.MODIFIEDNAME MODIFIEDNAME, ENRERENTSTATUS.MODIFIEDDATE MODIFIEDDATE, " +
      "ENRERENTSTATUS.DELETEDID DELETEDID, ENRERENTSTATUS.DELETEDNAME DELETEDNAME, " +
      "ENRERENTSTATUS.DELETEDDATE DELETEDDATE " +
      "FROM OSUSR_X6Q_RERENTSTATUS ENRERENTSTATUS " +
      "WHERE (ENRERENTSTATUS.ISACTIVE = 1) AND (ENRERENTSTATUS.NAME LIKE '%key%') " +
      "ORDER BY ENRERENTSTATUS.ID DESC LIMIT 5 OFFSET @offset";
    ...
    sqlCommand =
      ...
      "ENRERENTSTATUS.DELETEDDATE DELETEDDATE " +
      "FROM OSUSR_X6Q_RERENTSTATUS ENRERENTSTATUS " +
      "WHERE (ENRERENTSTATUS.ISACTIVE = 1) " +
      "ORDER BY ENRERENTSTATUS.ID DESC LIMIT 5 OFFSET @offset";
  ...
  if (key != "") {
    countsql =
      "SELECT COUNT(ENRERENTSTATUS.ID) AS Count " +
      "FROM OSUSR_X6Q_RERENTSTATUS ENRERENTSTATUS " +
      "WHERE (ENRERENTSTATUS.ISACTIVE = 1) AND (ENRERENTSTATUS.NAME LIKE '%key%') " +
      "ORDER BY ENRERENTSTATUS.ID DESC";
...
exports.updateReRentStatusById = async (req, res) => {
  const jsonData = JSON.parse(JSON.stringify(req.body));
  const timestamp = new Date().toISOString().slice(0, 19).replace("T", " ");
  const sqlCommand = "UPDATE OSUSR_X6Q_RERENTSTATUS SET ? WHERE ? ";
...
exports.deleteReRentStatusById = async (req, res) => {
  const jsonData = JSON.parse(JSON.stringify(req.body));
  const timestamp = new Date().toISOString().slice(0, 19).replace("T", " ");
  const sqlCommand = "UPDATE OSUSR_X6Q_RERENTSTATUS SET ? WHERE ? ";
...
exports.insertReRentStatus = async (req, res) => {
  const jsonData = JSON.parse(JSON.stringify(req.body));
  const timestamp = new Date().toISOString().slice(0, 19).replace("T", " ");
  const sqlCommand = "INSERT INTO OSUSR_X6Q_RERENTSTATUS SET ? ";
