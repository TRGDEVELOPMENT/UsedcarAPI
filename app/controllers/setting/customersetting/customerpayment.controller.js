...
exports.getCustomerPaymentList = async (req, res) => {
  ...
  if (key != "") {
    sqlCommand =
      "SELECT ENCUSTOMERPAYMENT.ID ID, ENCUSTOMERPAYMENT.CODE CODE, ENCUSTOMERPAYMENT.NAME NAME, " +
      "ENCUSTOMERPAYMENT.DESCRIPTION DESCRIPTION, CASE WHEN ENCUSTOMERPAYMENT.ISACTIVE = TRUE THEN 1 ELSE 0 END AS ISACTIVE, " +
      "ENCUSTOMERPAYMENT.CREATORID CREATORID, ENCUSTOMERPAYMENT.CREATORNAME CREATORNAME, " +
      "ENCUSTOMERPAYMENT.CREATEDDATE CREATEDDATE, ENCUSTOMERPAYMENT.MODIFIEDID MODIFIEDID, " +
      "ENCUSTOMERPAYMENT.MODIFIEDNAME MODIFIEDNAME, ENCUSTOMERPAYMENT.MODIFIEDDATE MODIFIEDDATE, " +
      "ENCUSTOMERPAYMENT.DELETEDID DELETEDID, ENCUSTOMERPAYMENT.DELETEDNAME DELETEDNAME, " +
      "ENCUSTOMERPAYMENT.DELETEDDATE DELETEDDATE " +
      "FROM OSUSR_X6Q_CUSTOMERPAYMENT ENCUSTOMERPAYMENT " +
      "WHERE (ENCUSTOMERPAYMENT.ISACTIVE = 1) AND (ENCUSTOMERPAYMENT.CODE LIKE '%@key%' OR ENCUSTOMERPAYMENT.NAME LIKE '%@key%') " +
      "ORDER BY ENCUSTOMERPAYMENT.ID DESC LIMIT 5 OFFSET @offset";
    ...
    sqlCommand =
      ...
      "ENCUSTOMERPAYMENT.DELETEDDATE DELETEDDATE " +
      "FROM OSUSR_X6Q_CUSTOMERPAYMENT ENCUSTOMERPAYMENT " +
      "WHERE (ENCUSTOMERPAYMENT.ISACTIVE = 1) " +
      "ORDER BY ENCUSTOMERPAYMENT.ID DESC LIMIT 5 OFFSET @offset";
  ...
  if (key != "") {
    countsql =
      "SELECT COUNT(ENCUSTOMERPAYMENT.ID) AS Count " +
      "FROM OSUSR_X6Q_CUSTOMERPAYMENT ENCUSTOMERPAYMENT " +
      "WHERE (ENCUSTOMERPAYMENT.ISACTIVE = 1) AND (ENCUSTOMERPAYMENT.CODE LIKE '%@key%' OR ENCUSTOMERPAYMENT.NAME LIKE '%@key%') " +
      "ORDER BY ENCUSTOMERPAYMENT.ID DESC";
...
exports.updateCustomerPaymentById = async (req, res) => {
  const jsonData = JSON.parse(JSON.stringify(req.body));
  const timestamp = new Date().toISOString().slice(0, 19).replace("T", " ");
  const sqlCommand = "UPDATE OSUSR_X6Q_CUSTOMERPAYMENT SET ? WHERE ? ";
...
exports.insertPayment = async (req, res) => {
  const jsonData = JSON.parse(JSON.stringify(req.body));
  const timestamp = new Date().toISOString().slice(0, 19).replace("T", " ");
  const sqlCommand = "INSERT INTO OSUSR_X6Q_CUSTOMERPAYMENT SET ? ";
